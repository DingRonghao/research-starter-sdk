...
当前发布版本：**1.0.0-internal.1**（内部测试 1.0）。
...
本地配置位于 `config.local.json`。首次启动若文件不存在，Launcher 会从发布模板创建它，并优先使用已配置的基础 Python；路径无效时只探测本机已有的 Python 3.12、3.11 或 3.10，不会下载新的 Python，也不会修改基础解释器。项目运行始终使用相对位置 `.venv/Scripts/python.exe`。Docling、Skills 和 Node 包均从项目目录解析。
